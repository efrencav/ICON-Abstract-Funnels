...
(function ($) {
  ...
  // setCurrentYear area start here ***
  function initActiveStatusPosition() {
    const nav = document.querySelector(".main-menu nav");
    if (!nav) return;

    const status = nav.querySelector(".activ-status");
    if (!status) return;
    ...
    // Function to position .activ-status horizontally centered on target li
    const setStatusPosition = (target) => {
      if (!target) return;
      ...
      status.style.left = `${centerX}px`;
    };

    // Initially position on active li
    setStatusPosition(activeItem);

    // On hover, move to hovered li center
    listItems.forEach((li) => {
      li.addEventListener("mouseenter", () => {
        setStatusPosition(li);
      });
